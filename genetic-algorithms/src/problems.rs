pub mod knapsack {
    ...
    pub fn knapsack(items: Vec<(usize, usize)>, max_weight: usize, string: String) -> f64 {
        trace!("items: {:?}", items);

        let (weight, value) = items             // get the vector of weight value pairs
            .iter()                             // get an iterator from that vector
            .zip(string.chars())                // zip the iterator elements together with the characters
                                                // of the string, producing another iterator
            .filter(|(_, b)| b == &'1')         // filter out the items without a '1' character
            .fold(                              // sum together the weights and values
                (0, 0),                         // the initial values
                |(weight, value), ((w, v), _)|  // for each iterator, take in:
        ...
        let fitness = if weight > max_weight { -1.0 } else { value as f64 / max_weight as f64 };
        debug!("weight limit: {max_weight}, weight: {weight}, value: {value}, fitness: {fitness}");
